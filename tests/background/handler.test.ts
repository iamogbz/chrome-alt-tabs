...
const undoLimitSpy = jest
  .spyOnProp<{ UNDO_LIMIT: number }>(constants, "UNDO_LIMIT")
  .mockValue(1);
...
describe("handler", () => {
  ...
  it("performs move to existing window", async () => {
    ...
    expect(moveTabsToWindowSpy).toHaveBeenCalledWith(mockTabs, targetWindowId);
  ...
  it("undoes last action", async () => {
    ...
    expect(chromeUtils.focusOnWindow).toHaveBeenLastCalledWith(sourceWindowId);
  });
  ...
  it("undoes only up to limit", async () => {
    ...
    await doMove().then(doMove).then(doMove);
    ...
    await undoMove().then(undoMove).then(undoMove);
    expect(moveTabsToWindowSpy).toHaveBeenCalledTimes(2);
    expect(moveTabsToWindowSpy).toHaveBeenLastCalledWith(
      mockTabs,
      sourceWindowId
